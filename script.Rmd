...
```{r}
posmo_boden <- st_intersection(posmo, boden_cropped)
```
...
```{r}
posmo_dtm <- posmo_boden |> 
  mutate(hoehe = extract(sdf, posmo_boden, fun=mean))

ggplot()+
  geom_point(data= posmo_dtm, aes(datetime, hoehe$altivrt))+
  geom_path(data= posmo_dtm, aes(datetime, hoehe$altivrt))
