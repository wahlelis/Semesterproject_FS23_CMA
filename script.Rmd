...
ggplot()+
  geom_point(data= posmo_annotated, aes(datetime, hoehe$altivrt, color = BBARTT_BEZ))+
  geom_path(data= posmo_annotated, aes(datetime, hoehe$altivrt))
```
...
```{r}
posmo_height <- posmo_dtm |> 
  mutate(
    h_minus1 = (lag(X)-X)^2 + (lag(Y)-Y)^2),
    h_minus2 = (lag(X,2)-X)^2 + (lag(Y,2)-Y)^2),
    h_minus3 = (lag(X,2)-X)^2 + (lag(Y,2)-Y)^2),
    h_plus1 = difftime(lead(datetime, 2), datetime),
    h_plus2 = difftime(lag(datetime), datetime),
    h_plus3 = difftime(lag(datetime, 2), datetime),
  )
```
