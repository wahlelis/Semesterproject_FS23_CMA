...
visualisation of average speed
...
posmo_segments <- posmo_transport |> 
    mutate(static = !is.finite(posmo_transport$speedmean_ms))
...
will look a bit closer at the exact speed during this time**
...
# zooming in the map, we see that the cable car is excluded in the allmiried area. since the walking speed of 4-5kmh is also excluded, the transport mode "walking" is preserved. however, some points on the mountainbike descent are excluded, probably because they fall in the speed range of the cable car.
...
mountainbike <- c("Acker, Wiese, Weide", "Fels", "geschlossener Wald",  "übrige bestockte", "übrige humusierte")
...
posmo_annotated <- posmo_annotated |>
  rowwise() |>
  mutate(elevmean = mean(c(h_minus1, h_minus2, h_minus3,
                      h_plus1, h_plus2, h_plus3), na.rm=TRUE), 
         elevsum = sum(c(h_minus1, h_minus2, h_minus3,
                      h_plus1, h_plus2, h_plus3), na.rm=TRUE)) |>
  ungroup()
```
