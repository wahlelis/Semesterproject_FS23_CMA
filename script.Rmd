...
posmo_transport <- posmo_transport |> 
  mutate(
    ...
    s_plus1 = as.numeric(t_plus1)/d_plus1, # s for speed [m/s]
    s_plus2 = as.numeric(t_plus2)/d_plus2,
    s_minus1 = -as.numeric(t_minus1)/d_minus1,
    s_minus2 = -as.numeric(t_minus2)/d_minus2
  )
...
posmo_transport <- posmo_transport |>
  ...
  mutate(
    speedmean = mean(c(s_plus1, s_plus2, s_minus1, s_minus2))
  ) |> 
...
ggplot(posmo_transport, aes(speedmean))+
  ...
  geom_vline(xintercept = mean(posmo_transport$speedmean, na.rm=TRUE))

range(posmo_transport$speedmean, na.rm=TRUE)

posmo_transport$speedmean # how did i get inf values? something divided by 0 returns inf. 0/0 returns NaN
pt <- posmo_transport$speedmean == Inf
length(pt[pt == TRUE]) # counts also NA values
```
