...
labelled as "car", whereas some points are labelled as "walk". We now
want to try to find out some criteria in order to be able to isolate
the downhill biking part and label it as "downhill biking"
...
Similar to Laube et al. (2011) we calculated the speed between two fixes.  for each point of the dataset the average speed of the 


```{r}
# plot / explore what we just did
ggplot(posmo_transport, aes(speedmean))+
  geom_histogram(binwidth = 1)+
  geom_vline(xintercept = mean(posmo_transport$speedmean, na.rm=TRUE)) #why is there no vline?

# no vline because of nan values
...
# of 2,237 rows only 80 rows are affected (3.58 %). since the +-Inf, NaN and NA values emerge where either the distance or the time between two points is 0, the points in question seem to be static (either temporally or locally). where no speed can be computed, no evaluation regarding a speed threshold can be taken. 

# new dataset for segmentation
...
ggplot(posmo_transport, aes(speedmean))+
  geom_histogram(binwidth = 1)+
  geom_vline(xintercept = mean(posmo_transport$speedmean)) # vline appears now. average speed around 10 m/s
