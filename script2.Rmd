...
wirie0518 <- wirie0518_alti |> 
  select(datetime, ID, mtb_speed, mtb_gc, mtb_elevation, geometry) |> 
  rowwise() |> 
...
wirie0518_summary <- wirie0518 |> 
  group_by(ID) |> 
  summarise(criteria3 = sum(mtb==3),
            criteria2 = sum(mtb==2),
            criteria1 = sum(mtb==1),
            tourtime=difftime(last(datetime), first(datetime)))

# ggplot(wirie0518_summary)+
#   geom_col( aes(criteria3))
...
wirie0507_speed$speedmean_ms[1:50] # how did i get inf and NaN values? something divided by 0 returns inf. 0/0 returns NaN. no vline because of nan values
...
wirie0507_alti <- wirie0507_gc |> 
  mutate(hoehe = extract(alti, wirie0507_gc)) # extract values froam a Raster for a set of locations.
...
wirie0507 <- na.omit(try)

tm_shape(wirie0507)+
  tm_dots("mtb", palette= brewer.pal(4, "BrBG"))

length(wirie0507_alti$mtb_speed[wirie0507_alti$mtb_speed == TRUE]) # 234 true (of 1015 rows)
length(wirie0507_alti$mtb_gc[wirie0507_alti$mtb_gc == TRUE]) # 513 true
...
valbirse_alti <- valbirse_gc |> 
  mutate(hoehe = extract(alti, valbirse_gc)) # extract values froam a Raster for a set of locations.
...
valbirse <- na.omit(try)

tm_shape(valbirse)+
  tm_dots("mtb", palette= brewer.pal(4, "BrBG"))

length(valbirse$mtb_speed[valbirse$mtb_speed == TRUE]) # 184 true (of 1015 rows)
length(valbirse$mtb_gc[valbirse$mtb_gc == TRUE]) # 297 true
...
length(valbirse$mtb_count3[valbirse$mtb_count3 == TRUE]) # x true
