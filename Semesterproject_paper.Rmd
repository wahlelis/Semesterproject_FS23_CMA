...
```{r rename / overwrite rows of points within the speed range of mountainbiking, include=FALSE}
...
```{r new segmentation, include=FALSE}
...
wirie0518_try <- wirie0518_try |>
  filter(lastsegm==TRUE) # select rows where the value is TRUE to select the downhill mountainbiking segments
...
the pattern detected with criteria. segments 
...
tm_shape(wirie0518)+
  ...
  tm_view(set.view = c(7.53381, 46.60840, 14))
```

```{r "speed movement parameter plots", echo=FALSE}
