...
wirie0518_try <- wirie0518_try |>
  filter(mtb_refined==TRUE) # select rows where the value is TRUE to select the downhill mountainbiking segments
...
```{r wirie 0507 rename overwrite rows of points within the speed range of mountainbiking, include=FALSE}
...
ggplot(wirie0507_try, aes(X, Y))+
  geom_path(aes(color=ID_mtb))+
  geom_point(aes(color = "mtb_refined"))+
  coord_fixed()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
